    ...
    , m_iCurrentLevel(0)
{
    ...
    // ✅ Di chuyển Initialize sau khi cửa sổ đã được tạo
...
void Game::run() {
    ...
    while (m_Window.isOpen()) {
        ...
        case Play:
            // Đã sửa đổi: Cấu trúc cập nhật hợp lý hơn
            if (m_MenuManager.IsInGamePlay()) {
                ...
                // Khi không ở trong gameplay, tức là đang ở menu chính hoặc menu profile
                m_MenuManager.Update(m_Window, m_deltaTime.asSeconds());
            }
            break;
        }
...
void Game::HandleInput() {
    ...
    while (m_Window.pollEvent(event)) {
        ...
        case Play:
            // Đã sửa đổi: Cấu trúc xử lý input hợp lý hơn
            if (m_MenuManager.IsInGamePlay()) {
                if (m_MenuManager.IsGamePaused()) {
    ...
    // Handle continuous keyboard input only in play mode
    if (m_eGameMode == Play && m_MenuManager.IsInGamePlay() && !m_MenuManager.IsGamePaused()) {
        HandleKeyboardInput();
    }
}
