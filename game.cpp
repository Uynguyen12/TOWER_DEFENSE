    ...
    , m_fDifficulty(1.0f)
{
    ...
    m_PlayerText.setPosition(sf::Vector2f(2200, 100));
...
void Game::UpdatePlay() {
    ...
    //float fLengthFromAxeToEnemy = MathHelpers::flength(vAxeToEnemy);

    ////Move the enemy to the random spot when the axe hit the enemy
    //if (fLengthFromAxeToEnemy < 160.0f) {
    //    //The axe hit the enemy
    //    sf::Vector2f vNewPosition(rand() % 2560 * 1.0f, rand() % 1600 * 1.0f);
...
void Game::DrawPlay() {
    ...
    m_PlayerText.setString("Player's Health: " + to_string(m_iPlayerHealth) + "\nPlayer's Gold: " + to_string(m_iPlayerGold));
...
void Game::HandleInput() {
    ...
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Y)) {
        if (!bYwasPressedLastUpdate) {
			m_bDrawPath = !m_bDrawPath; // Toggle the path drawing
        }
