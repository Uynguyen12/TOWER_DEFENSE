...
Game::Game()
    ...
    , m_bGameRunning(true)
    , m_bGameOverSoundPlayed(false) // Khởi tạo biến trạng thái âm thanh
    , m_iCurrentLevel(0) // Khởi tạo level hiện tại
{
    ...
    // Initialize SoundManager and load background music
    SoundManager::getInstance().Initialize();
    // Nhạc nền sẽ được phát khi vào game, không cần phát ở đây ngay.
    ...
    m_MenuManager.SetStartGameCallback([this](int level) {
        this->StartGame(level); // 'this' là con trỏ tới đối tượng Game hiện tại
    });

    // Load textures for templates (can be done later in StartGame/LoadLevel if preferred)
    ...
    while (m_Window.isOpen()) {
        ...
        if (!m_MenuManager.IsInGamePlay()) {
            ...
            if (!m_MenuManager.IsGamePaused()) {
                ...
                // If game is paused, update the pause menu
                m_MenuManager.Update(m_Window, m_deltaTime.asSeconds());
            }
        }
        Draw();
...
void Game::LoadLevel(int level) {
    /// Định dạng tên file level (ví dụ: "levels/level1.txt")
    std::string levelFileName = "levels/level" + std::to_string(level) + ".txt";

    try {
        // 1. Tải texture atlas cho Map
        ...
        // Quay về menu chính nếu không load được level
        ReturnToMenu();
    }
    catch (...) { // Catch any other potential errors during loading
...
void Game::DrawPlay() {
    ...
    // Vẽ các enemy đang hoạt động
    for (const Entity& enemy : m_enemies) {
        m_Window.draw(enemy);
    }
    // Vẽ các projectiles (axes)
    for (const Entity& axe : m_axes) {
        m_Window.draw(axe);
    }
    // --- HẾT PHẦN CẦN THÊM ---

    // Vẽ tower placement preview
    m_Window.draw(m_TowerTemplate);

    // Update and draw player text
    UpdatePlayerText();
...
void Game::Draw() {
    ...
    if (!m_MenuManager.IsInGamePlay()) {
        ...
        // Vẽ UI thông tin game (chế độ, player stats)
        m_Window.draw(m_GameModeText);
        m_Window.draw(m_PlayerText);

        // Vẽ các thành phần game play hoặc level editor
        switch (m_eGameMode) {
        case Play:
            DrawPlay();
            break;
        ...
        // Nếu game đang pause, vẽ menu pause
        if (m_MenuManager.IsGamePaused()) {
            m_MenuManager.Draw(m_Window); // MenuManager sẽ tự vẽ pause menu khi cần
        }
        else if (m_iPlayerHealth <= 0) {
            // Vẽ màn hình Game Over nếu health <= 0 và không pause
            m_Window.draw(m_GameOverText);
        }
    }
    m_Window.display();
...
void Game::StartGame(int level) {
    if (level <= 0) { // Giả định level phải là số dương
        std::cerr << "Invalid level selected: " << level << std::endl;
        ReturnToMenu();
        return;
    }

    m_iCurrentLevel = level; // Lưu lại level hiện tại

    LoadLevel(m_iCurrentLevel); // Gọi hàm tải level dựa trên level được chọn

    // Reset lại trạng thái game cho level mới
    ResetGameState();
    // Đặt lại health và gold khởi đầu cho level mới
    m_iPlayerHealth = 10;
    m_iPlayerGold = 10;

    // Cập nhật UI và chế độ game
    m_eGameMode = Play;
    m_GameModeText.setString("Play Mode");

    // Bắt đầu nhạc nền game nếu chưa chạy hoặc dừng nhạc menu
    SoundManager::getInstance().StopBackgroundMusic(); // Đảm bảo nhạc menu đã dừng
    SoundManager::getInstance().PlayBackgroundMusic(); // Phát nhạc game

    // Chuyển trạng thái MenuManager sang Gameplay
    m_MenuManager.SetMenuState(MenuManager::MenuState::GamePlay);

    // Cập nhật Player Text ngay lập tức sau khi reset game state
    UpdatePlayerText();
}
...
void Game::ResetGameState() {
    ...
    // Reset các biến trạng thái game play
    m_iGoldGainedThisUpdate = 0;
    m_fTimeInPlayMode = 0.0f;
    m_fDifficulty = 1.0f;
    m_fGoldPerSecond = 0.0f;
    m_fGoldPerSecondTimer = 0.0f;
    m_bGameRunning = true;
    m_bGameOverSoundPlayed = false; // Reset trạng thái âm thanh game over

    // Reset các tile trong Map (có thể cần thiết nếu Map giữ trạng thái độc lập)
    // m_Map.ClearTiles(); // Đảm bảo Map cũng sạch sẽ

    // Đặt lại vị trí các UI text nếu cần
    m_GameModeText.setString("Play Mode");
    m_PlayerText.setString("Player Gold: " + std::to_string(m_iPlayerGold) + "\nHealth: " + std::to_string(m_iPlayerHealth));
    // GameOverText sẽ chỉ hiển thị khi m_iPlayerHealth <= 0
}
