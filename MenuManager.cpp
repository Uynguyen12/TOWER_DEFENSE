...
void MenuManager::CreatePlayMenu() {
    ...
    for (int i = 1; i <= 4; ++i) {
        ...
        CreateButton(levelText,
            ...
            sf::Vector2f(BUTTON_WIDTH, BUTTON_HEIGHT),
            [this, i]() {
                if (m_startGameCallback) { // Nếu callback đã được set
                    m_startGameCallback(i); // Gọi callback với level index
                }
            });
    }
...
void MenuManager::SetStartGameCallback(std::function<void(int)> callback) {
    m_startGameCallback = callback; // Lưu callback được cung cấp bởi Game
}